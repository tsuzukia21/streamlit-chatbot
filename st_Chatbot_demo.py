...
_LOREM_IPSUM = "Lorem ipsum dolor sit amet, **consectetur adipiscing** elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quisnostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat."
...
def st_Chat_demo():
    ...
    with st.sidebar.container():
        engine = st.selectbox("model",("demo","gpt-4-turbo","gpt-4o","gpt-3.5-turbo","claude-3-opus","gemini-1.5-pro"),help="You can select the model.")
    ...
    if prompt := st.chat_input(placeholder="Send a message"):
        ...
        st.session_state.prompt = prompt.upper()
        st.session_state.edit = False
        ...
        with st.chat_message("assistant", avatar="🤖"):
            ...
            with col1:
                # Commenting out the chain.stream() and using st.write_stream() to output uppercase prompt
                ...
                demo_txt = st.write_stream(stream_data)
                st.session_state.done = True
                st.session_state.last_response = demo_txt.replace("\n", "\\n").replace('"', '\\"')
                st.session_state.memory.save_context({"input": prompt}, {"output": demo_txt})
                st.session_state.messages = st.session_state.memory.buffer
        st.session_state.Clear = True
        st.rerun()
    ...
    if st.session_state.save:
        st.session_state.Clear = False
        st.session_state.done = False
        prompt = st.session_state.new_message.upper()
        ...
        with st.chat_message("assistant", avatar="🤖"):
            ...
            with col1:
                ...
                demo_txt = st.write_stream(stream_data)
                st.session_state.done = True
                message_placeholder.markdown(demo_txt.replace("\n", "  \n"), unsafe_allow_html=True)
                st.session_state.last_response = demo_txt.replace("\n", "\\n").replace('"', '\\"')
                st.session_state.memory.save_context({"input": prompt}, {"output": demo_txt})
                st.session_state.messages = st.session_state.memory.buffer
