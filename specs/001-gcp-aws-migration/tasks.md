...
- [ ] T013 [P] [US1] `update_conversation_timestamp(conversation_id)` を DynamoDB `update_item` で実装 (core/database.py)
- [ ] T014 [US1] `delete_conversation(conversation_id)` を is_deleted=True に設定する DynamoDB `update_item` で実装 (core/database.py)
- [ ] T015 [US1] `save_message(conversation_id, role, content, reasoning)` を DynamoDB `put_item` で実装（pk=CONV#<id>, sk=<timestamp>#<msgId>） (core/database.py)
- [ ] T016 [US1] `get_messages(conversation_id)` を DynamoDB `query` で実装（pk=CONV#<id>, sk > METADATA） (core/database.py)
